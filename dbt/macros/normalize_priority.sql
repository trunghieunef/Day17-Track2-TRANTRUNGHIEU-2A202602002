...
{% macro normalize_priority(col) %}
    case
        -- Nhóm 2: nhãn chuỗi do backend gửi từ 08-10 (ý nghĩa không đổi, chỉ
        -- đổi cách biểu diễn). Quy về số theo tài liệu API.
        when lower({{ col }}) = 'urgent'  then 1
        when lower({{ col }}) = 'high'    then 2
        when lower({{ col }}) = 'medium'  then 3
        when lower({{ col }}) = 'low'     then 4
        -- Nhóm 1: số hợp lệ trong miền 1..4, giữ nguyên.
        when try_cast({{ col }} as integer) between 1 and 4 then cast({{ col }} as integer)
        -- Nhóm 3: mọi giá trị khác ('0','5','-1','P1','unknown','' NULL,...)
        -- là dữ liệu hỏng thật -> NULL (tín hiệu để quarantine nhặt ra).
        else null
    end
{% endmacro %}
...
{% macro priority_reject_reason(col) %}
    case
        when {{ col }} is null or {{ col }} = '' then 'priority rỗng/NULL'
        when try_cast({{ col }} as integer) is null then 'priority không phải số: ' || {{ col }}
        else 'priority ngoài miền hợp lệ 1..4: ' || {{ col }}
    end
{% endmacro %}
